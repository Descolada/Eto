		...
		where TCallback : Window.ICallback
	{
		...
		private void Control_FormClosing(object sender, swf.FormClosingEventArgs e)
		{
				...
				|| e.CloseReason == swf.CloseReason.WindowsShutDown)
			{
				...
				// Fire Terminating so the app can veto QUITTING, but with a separate CancelEventArgs so a veto
				// (Cancel = true, "keep the app running") doesn't also cancel THIS window's own close. Letting a
				// Terminating veto keep the window open only makes sense for the MainForm; a transient/dialog
				// window must still close even when the app declines to quit -- otherwise a modal dialog gets
				// trapped in its ShowModal loop and its buttons appear dead.
